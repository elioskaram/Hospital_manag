    ...
    public void showPatientFrm(Stage s, Patient p) {
        ...
        EventHandler<ActionEvent> event = new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent e) {
                ...
                String titles = "ID type price option1 option2 \n";
    ...
    private void btnSubmit(Patient p) {
        if (valid()) {
            if (GFG.isValidTime(tfTime.getText())) {
                ...
                if (isDuplicateTestSelection(p, testID)) {
                    displayErrorMessage("Test already selected for the same date.");
                    return;
                }
                ...
                displayErrorMessage("Error input: Time format is wrong.");
            }
        } else {
            displayErrorMessage("Error input: Select the ID and Time.");
        }
    }

    private List<Reservation> patientReservations = new ArrayList<>();

// Define a Reservation class to store reservation details
class Reservation {
    int testID;
    LocalDate reservationDate;

    public Reservation(int testID, LocalDate reservationDate) {
        this.testID = testID;
        this.reservationDate = reservationDate;
    }
}

private boolean isDuplicateTestSelection(Patient p, int testID, LocalDate reservationDate) {
    // Check if the same test is already selected for the same date by the patient
    for (Reservation reservation : patientReservations) {
        if (reservation.testID == testID && reservation.reservationDate.equals(reservationDate)) {
    ...
    private boolean valid() {
        return (!cbTest.getSelectionModel().isEmpty() && !tfTime.getText().isEmpty());
    }
